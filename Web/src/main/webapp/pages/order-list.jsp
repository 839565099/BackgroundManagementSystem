...
<meta
	...
	<script type="text/javascript">
		function delOne_order(id) {
			if (confirm("是否删除这条记录~~~")){
				location.href="${pageContext.request.contextPath}/orders/delOne?id="+id;
			}
		}

		function delMany_order() {
			if (confirm("是否删除选中记录~~~")){
				$("#f1").submit()
			}
			...
			<div class="box box-primary">
				...
				<div class="box-body">
					...
					<div class="table-box">
						...
						<form id="f1" action="${pageContext.request.contextPath}/orders/delMany">
						<!--数据列表-->
						<table id="dataList"
							...
							<tbody>
                             <c:forEach items="${ordersList}" var="o" varStatus="i">
								<tr>
									...
									<td>${i.count}</td>
									<td>${o.orderNum}</td>
									<td><fmt:formatDate value="${o.orderTime}" pattern="yyyy-MM-dd"></fmt:formatDate></td>
									<td>${o.peopleCount}</td>
									<td>${o.payType==1?"支付宝":o.payType==0?"微信":"其他"}</td>
									...
									<td class="text-center">
										...
										<button type="button" class="btn bg-olive btn-xs"
											onclick='delOne_order(${o.id})'>删除</button>
										<button type="button" class="btn bg-olive btn-xs"
